...
| 文件 | 内容 | 步骤 |
|------|------|------|
| `.d2c/OVERVIEW.md` | 项目名称、类型、目的、受众、元信息、设计源 URL | 步骤一 |
| `.d2c/PRD.md` | 业务目标、功能范围、用户流程、页面结构、成功指标 | 步骤二 |
| `.d2c/DESIGN.md` | 设计令牌、组件、动效、约束、响应式断点。纯文档。 | 步骤三 |
| `.d2c/ARC.md` | 前后端技术栈、部署策略、架构决策。核心原则：简单清晰。 | 步骤四 |
| `.d2c/SPEC.md` | 文件结构、环境变量、API 契约、组件树、编码约束、测试策略 | 步骤五 |
...
/d2c <输入>
  │
  ├─ 检测: Figma 链接 / 网页 URL / 图片 / 文件
  │
  ├─ 步骤一: OVERVIEW.md — 项目概要
  ├─ 步骤二: PRD.md — 业务需求
  ├─ 步骤三: DESIGN.md — 设计系统 & 令牌
  ├─ 步骤四: ARC.md — 架构 & 技术栈
  ├─ 步骤五: SPEC.md — 开发规格 & 约束
  │
...
# 步骤一：项目概要 → .d2c/OVERVIEW.md（元信息、类型、受众）
# 步骤二：业务需求 → .d2c/PRD.md（目标、功能、流程）
# 步骤三：设计系统 → .d2c/DESIGN.md（令牌、组件、动效）
# 步骤四：架构设计 → .d2c/ARC.md（技术栈、部署）
# 步骤五：开发规格 → .d2c/SPEC.md（文件结构、环境变量、API）
